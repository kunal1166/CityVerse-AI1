...
import { LayoutDashboard, Bus, CloudRain, BarChart3, FileText, GripVertical, Settings, Map, X } from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  const { activeTab, setActiveTab, dashboardData, isSidebarOpen, toggleSidebar } = useCityStore();
  ...
  const handleSelectTab = (tabId: TabType) => {
    setActiveTab(tabId);
    // Auto-close sidebar on mobile viewports upon selection
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 lg:static lg:z-20 w-64 lg:w-56 bg-white dark:bg-slate-900 border-r border-gray-200 dark:border-slate-700 flex flex-col justify-between shrink-0 select-none shadow-xl lg:shadow-none transition-transform duration-300 ease-in-out">
        {/* Navigation Links */}
        <div className="p-3 space-y-1 overflow-y-auto">
          <div className="px-2 py-1 flex items-center justify-between text-[10px] font-semibold text-gray-400 dark:text-slate-500 uppercase tracking-wider">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={toggleSidebar}
                className="p-1 rounded text-gray-400 hover:text-gray-600 dark:hover:text-slate-200 lg:hidden"
              ...
              return (
                ...
                >
                  <div
                    ...
                    className={`group w-full flex items-center justify-between px-2.5 py-2.5 lg:py-2 rounded-md text-xs font-medium cursor-pointer transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
                      isActive
                  ...
                  >
                    <div className="flex items-center space-x-2.5 lg:space-x-2 min-w-0">
                      ...
                      <span className="truncate text-sm lg:text-xs">{item.label}</span>
                    </div>
                    {item.badge && (
                      <span
